...
export const InputImage: FC<PropsInputImage> = ({ image, changeImageFile }) => {
  return (
    <label className="ms-4 me-16 flex flex-col items-center">
      ...
      <div className={`mt-2 w-40 aspect-video rounded flex items-center justify-center cursor-pointer ${image ? "border-none" : "border-2 border-dashed"}`}>{image ? <img src={image} alt={image} /> : <span>Select Image</span>}</div>
    </label>
  )
}
